...
라플라스 메커니즘은 무한한 범위를 가지므로 음수 카운트처럼 의미상 불가능한 값을 반환할 수 있습니다.[^2]
...
비음수와 합 일관성 같은 제약을 만족하는 사후 추정을 쓰면 같은 ε에서도 정확도를 높일 수 있습니다.[^10]

미국 2020 센서스의 TopDown 알고리즘도 노이즈가 섞인 값을 그대로 공개하지 않았습니다. 비음수, 정수, 계층 합일관성 제약을 강제하는 대규모 후처리를 적용했습니다. 다만 외부 평가에서는 이런 후처리가 인구가 적은 지역에서 체계적인 양의 편향을 만들고, 주 인구 합 같은 불변값이 고정된 큰 지역에서는 반대로 음의 편향을 만들 수 있다는 점이 확인됐습니다.[^11]
