...
차등 프라이버시는 어떤 결과를 보고 특정 개인이 데이터에 있었는지 알아내는 능력에 **ε**(엡실론)이라는 상한을 씌우는 기법입니다. NIST SP 800-226도 차등 프라이버시를 개인의 데이터가 데이터셋에 있을 때 생기는 프라이버시 손실을 정량화하는 수학적 프레임워크로 설명합니다.[^nist]
...
대표 사례가 코시(Cauchy) 분포입니다. 생긴 것은 종 모양이지만 꼬리가 아주 두껍습니다. NIST의 통계 핸드북도 코시 분포의 평균과 표준편차는 정의되지 않으며, 1,000개 데이터를 모아도 평균 추정이 한 점으로 안정되지 않는다고 설명합니다.[^cauchy]
...
하지만 두꺼운 꼬리를 자르면 평균이 치우칠 수 있습니다. 자르지 않으면 노이즈가 커지고, 자르면 편향이 생깁니다. Kamath, Singhal, Ullman의 연구도 무거운 꼬리 분포의 비공개 평균 추정에서 이런 표본 수와 정확도 상충이 핵심임을 분석합니다.[^ksu]
...
프라이버시를 지키면 정확도에 비용이 생깁니다. 이 비용은 단순한 감이 아니라 정리로 증명됩니다. Dwork와 Roth의 표준 교과서도 여러 기본 질의에서 프라이버시 보장과 정확도 사이의 하한을 다룹니다.[^dr]
...
고차원에서는 비용이 더 무겁습니다. 좌표가 하나일 때와 100개일 때는 같은 문제가 아닙니다. Hardt와 Talwar는 선형 질의의 노이즈 복잡도가 질의의 기하학적 구조에 의해 결정된다는 점을 보였고, 순수 ε-DP와 조금 약한 (ε, δ)-DP 사이의 차이도 분명히 드러냈습니다.[^ht]
...
질의가 아주 많아질 때는 핑거프린팅 하한이 등장합니다. Bun, Ullman, Vadhan은 핑거프린팅 코드를 이용해, 많은 카운팅 질의에 동시에 정확히 답하려면 표본 수가 충분히 커야 한다는 정보이론적 하한을 보였습니다.[^buv]
...
같은 데이터에 쿼리를 반복하면 ε이 누적됩니다. Dwork와 Roth의 합성 정리는 여러 차등 프라이버시 메커니즘을 함께 공개할 때 전체 보장이 어떻게 쌓이는지 설명합니다.[^dr] NIST도 프라이버시 예산을 하나의 데이터셋을 처리하는 모든 분석에 걸친 누적 손실의 허용 상한으로 정의합니다.[^nist]
...
머신러닝에서 흔한 홀드아웃 검증도 조심해야 합니다. 검증 세트를 한두 번 쓰는 것과, 결과를 보고 계속 다시 쓰는 것은 다릅니다. Dwork 연구진은 적응적으로 반복되는 데이터 분석에서 통계적 타당성이 무너지는 문제를 다뤘고, 차등 프라이버시 기법이 이런 재사용 위험을 제어하는 데 쓰일 수 있음을 보였습니다.[^adaptive]
