...
한 주(州) 인구통계학자가 인구총조사 표를 열어 보니, 인구가 적은 어느 카운티에서 5세 미만 아동 수가 실제보다 평균 1.53명씩 적게 잡혀 있었습니다.
...
인구조사국은 데이터 스와핑이 이런 공격을 막도록 설계된 적이 없으며, 막을 만큼 노이즈를 넣으면 데이터가 쓸 수 없을 정도로 부정확해진다고 판단했습니다.[^3] 그래서 트레이드오프를 수치로 다룰 수 있는 형식적 프라이버시, 곧 차등 프라이버시(DP)로 갈아탑니다.
...
다음으로 이 측정값들을 모아, 한 사람과 한 주택을 한 줄씩 담은 마이크로데이터로 다시 짜 맞춥니다.[^4]
...
핸드북의 설명이 직관적입니다. 한 카운티 안 모든 트랙의 노이즈 인구를 본 뒤, 각 트랙 값을 자기 노이즈에 가장 가까우면서도 카운티 총인구에 합산되도록 정한다는 것입니다.[^5]

이렇게 하면 블록 표를 모으면 블록 그룹 표가 되고, 그것을 모으면 트랙 표가 되는 식으로 계층 간 숫자가 어긋나지 않습니다.[^6]

위에서 아래로 가는 이유는 정확도 때문입니다. 블록을 따로따로 추정해 위로 더하는 것보다, 인구조사 지리의 나무 구조를 통째로 활용하는 편이 더 정밀한 추정을 준다는 것입니다.[^7]
...
인구가 음수일 수는 없으니, 음수를 없애고 정수로 맞추는 보정이 필요합니다. 이 단계가 사후 보정(post-processing)입니다. 이미 나온 노이즈 측정값을 손봐 비음수·정수·계층 일관성 같은 제약을 만족시키는 작업입니다.[^9]

인구조사국은 이 지점을 두고 "DAS는 교과서적 차등 프라이버시에서 한 가지 중요한 방식으로 벗어난다"고 직접 적었습니다.[^10] 모든 인구·주택 카운트는 정수여야 하고 음수일 수 없으며, 특정 불변량(invariant)은 노이즈 없이 그대로 둔다는 제약이 추가되기 때문입니다.[^10]

여기서 함정이 생깁니다. 사후 보정은 깔끔한 ε 보장의 바깥에서 일어나는 계산입니다.

인구조사국 설계자(Abowd와 Hawes)조차 비음수 제약이 "정확도의 범인"이며, 이 제약이 만든 오차는 프라이버시 예산(ε)으로 합리적으로 통제할 수 없다고 인정합니다.[^11]
...
그러면 257을 254로 끌어내려야 합니다. 부모 지역과의 일관성 제약 때문에, 3명만큼을 어딘가에서 깎아 블록들에 다시 나눠야 합니다.[^12]
...
Santos-Lozada 연구팀(PNAS 2020)은 사망률을 다시 계산해 보았습니다. 일부 경우 2010년 원본 인구로 계산한 사망률이 DP 보호 인구로 계산한 값의 두 배까지 차이 났습니다.[^15] 분모인 인구가 흔들리면 비율 지표가 통째로 흔들린다는 뜻입니다.
...
후속 연구(Mueller & Santos-Lozada 2022)는 카운티 단위에서 이 패턴을 확인했습니다. 도시에서 농촌으로 갈수록 추정 정확도가 꾸준히 떨어졌는데, 유일한 예외가 비히스패닉 백인 인구였습니다.[^18] 결과적으로 백인 인구가 가장 정확한 카운트를 받는 구조가 됩니다.
...
가장 격렬한 비판은 Imai 팀(Kenny·Kuriwaki·McCartan·Rosenman·Simko·Imai)에게서 나왔습니다.
...
먼저 사실관계를 짚어야 합니다. 흔한 오해와 달리, 블록 단위 총인구는 불변량이 아닙니다. 노이즈 없이 그대로 보존되는 값은 의도적으로 짧은 목록입니다. 각 주와 DC·푸에르토리코의 총인구, 각 블록의 주택 호수, 각 블록의 점유된 집단 거주시설 수와 종류뿐입니다.[^25]

다시 강조하면, 블록의 주택 "호수"는 불변량이지만 그 안에 사는 "사람 수"는 불변량이 아닙니다.[^25]

그래서 반론은 블록 인구 불변량에 기대지 않습니다. 두 기둥으로 섭니다.

첫째, 노이즈 측정값 자체는 무편향입니다. 이산 가우시안 메커니즘에서 노이즈 측정값의 기대 오차는 0이므로, 체계적 편향은 프라이버시 메커니즘이 아니라 하류의 비음수·사후 보정 탓이라는 것입니다.[^26]
...
Abowd의 후속 논의는 한 발 더 나갑니다. 무편향 노이즈 측정값을 선거구 규모로 모으면, 음수 항이 많더라도 합리적인 선거구를 만들 수 있고, 그 집계의 분산은 구성 통계들의 분산을 더해 정확히 계산할 수 있다는 것입니다.[^28]

질의 전략이 질의 수요와 맞으면, 2020년 선거구 데이터 어떤 집계든 그 불확실성을 독립 통계들의 분산 합으로 직접 구할 수 있다는 주장도 같은 맥락입니다.[^29] 통제 불가능한 사후 보정 오차 대신, 계산 가능한 불확실성을 사용자에게 주겠다는 것입니다.
...
설계자 본인이 정확도 손상의 위치를 차등 프라이버시 노이즈가 아니라 비음수 사후 보정 제약으로 콕 집었습니다.[^30] 0인 값을 0이 아닌 값으로 바꾸면서 비음수를 강제하는 시스템이라면, 과거의 억제나 스와핑을 포함해 무엇이든 같은 체계적 편향을 겪는다는 것입니다.[^30]

경계는 여기 있습니다. 논쟁의 대상은 형식적 프라이버시 틀 자체가 아니라, 특정 사후 보정과 공표 제품 설계의 선택입니다. Abowd 2023~24년 논문의 제목이 그 경계를 그대로 말합니다. 노이즈 측정값도 중요하지만, 공표되는 인구총조사 제품의 설계가 정확도에 훨씬 더 중요하다는 것입니다.[^31]

아직 결판나지 않은 전제도 따로 있습니다. 스와핑을 버리게 만든 재구성 위협이 정말 심각했는가 하는 물음입니다.

Ruggles 2025년 글이 정리한 Ruggles & Van Riper 2022년 분석에 따르면, 무작위로 뽑은 사람이 무작위로 고른 블록의 누군가와 나이·성별이 정확히 일치할 확률이 52.6%였습니다.[^32] 인구조사국이 "성공"이라 부른 복원 일치의 상당수가 정보 유출이 아니라 우연으로 설명된다는 것입니다.
