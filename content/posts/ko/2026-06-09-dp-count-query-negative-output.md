---
title: "DP 카운트 쿼리에서 음수가 나오는 이유: 버그가 아니라 노이즈입니다"
description: "차등 프라이버시 카운트 쿼리에서 음수가 나오는 이유와, 0으로 자를 때 생기는 편향을 수식보다 직관과 실무 판단 기준으로 설명합니다."
date: "2026-06-09"
category: "Technology"
...
ε=0.1로 카운트를 뽑았더니 화면에 `-3.7`이 찍혔습니다.

환자 수가 음수라니, 누가 봐도 이상합니다. 첫 반응은 대개 이렇습니다.

"라이브러리 버그 아닌가?"

결론부터 말하면, 아닙니다. 라플라스 메커니즘에서는 음수 카운트가 정상적으로 나올 수 있습니다.

> **한 줄 요약**: 차등 프라이버시는 참값에 노이즈를 더합니다. 그 노이즈는 위로도, 아래로도 흔들리기 때문에 작은 카운트에서는 음수가 나올 수 있습니다. 0보다 작은 값을 0으로 자르면 ε 보장은 깨지지 않지만, 통계는 위로 치우칠 수 있습니다. 화면 표시용과 분석용 후처리는 구분해야 합니다.

NIST 공식 용어집은 라플라스 메커니즘을 쿼리 결과에 라플라스 분포에서 뽑은 무작위 노이즈를 더하는 차등 프라이버시 기본 부품으로 정의합니다.[^1] 이 정의에는 "출력은 반드시 0 이상이어야 한다"는 조건이 없습니다.

Holohan 외의 논문도 라플라스 메커니즘이 무한한 범위를 갖기 때문에 음수 카운트처럼 의미상 불가능한 값을 반환할 수 있다고 설명합니다.[^2]

OpenDP의 `make_laplace` 역시 노이즈를 더한다고 설명할 뿐, 비음수로 잘라 준다고 말하지 않습니다.[^3]
...
카운트 쿼리에서는 보통 한 사람이 들어오거나 빠질 때 결과가 최대 1만 바뀝니다. 그래서 보호를 강하게 걸수록, 즉 ε을 작게 잡을수록 더 큰 노이즈를 넣습니다.
...
ε=0.1처럼 보호를 강하게 걸면 참 카운트 5에서도 세 번에 한 번꼴로 음수가 나올 수 있습니다. 직관에는 어색하지만, 노이즈 분포 입장에서는 자연스러운 결과입니다.

실제 라이브러리 예시도 같은 흐름입니다. OpenDP의 typical workflow 튜토리얼은 카운트 쿼리 예제에서 `scale = 3.0`, `accuracy = 9.44`를 보여 줍니다.[^5] 작은 카운트라면 음수가 충분히 나올 수 있는 수준입니다.
...
McGlinchey와 Mason은 라플라스 메커니즘 출력에 0 자르기나 truncation을 적용하면 엄격한 양의 편향이 생긴다는 결과를 보였습니다.[^9]

작은 카운트가 많을수록 문제가 커집니다. 예를 들어 지역별 희귀질환 환자 수, 세부 진단명별 카운트, 연령×성별×지역 표처럼 셀이 잘게 쪼개진 통계에서는 음수가 자주 나옵니다.
...
Hay, Rastogi, Miklau, Suciu는 비음수와 부모-자식 합 일관성 같은 제약을 만족하는 사후 추정을 쓰면 같은 ε에서 정확도를 높일 수 있음을 보였습니다.[^10]

미국 2020 센서스의 TopDown 알고리즘도 노이즈가 섞인 값을 그대로 공개하지 않았습니다. 비음수, 정수, 계층 합일관성 제약을 강제하는 대규모 후처리를 적용했습니다. 다만 외부 평가에서는 이런 후처리가 작은 인구 지역에서 체계적인 양의 편향을 만들고, 주 인구 합 같은 불변값이 고정된 큰 지역에서는 반대로 음의 편향을 만들 수 있음이 확인됐습니다.[^11]
...
음수 카운트가 나왔을 때는 이렇게 판단하는 편이 안전합니다.

1. **먼저 음수 발생 빈도를 예상합니다.** 카운트가 작고 ε이 작으면 음수는 꽤 자주 나옵니다.
