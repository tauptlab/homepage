...
실제로 같은 연구팀은 macOS 구현에서 한 건당 프라이버시 손실이 ε으로 1 또는 2 수준이라고 실측했습니다.[^2] 우리가 보는 ‘한 자리 ε’은 한 조각의 데이터에 붙는 숫자라는 뜻입니다.
...
이 결과가 코드 수준에서도 탄탄한 이유가 하나 더 있습니다. 디스어셈블된 구현은 비트 뒤집힘 확률을 `1.0/(exp(PrivacyParameter) + 1.0)`이라는 상수로 계산했는데, 이는 Apple ML 백서의 노이즈 공식과 정확히 일치합니다.[^2] 역공학으로 끄집어낸 숫자가 곧 노이즈 양을 정하는 같은 ε임이 확인됩니다.
