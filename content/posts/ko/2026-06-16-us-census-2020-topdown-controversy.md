...
다음으로 이 측정값들을 모아, 사람과 주택을 한 줄에 하나씩 담은 마이크로데이터로 다시 짜 맞춥니다.[^4]
...
인구조사국 설계자들 스스로도 비음수 제약이 "정확도의 범인"이며, 이 제약이 만든 오차는 프라이버시 예산(ε)으로 합리적으로 통제할 수 없다고 인정했습니다.[^11]
...
첫째, 노이즈 측정값 자체는 무편향입니다. 이산 가우시안 메커니즘에서 노이즈 측정값의 기대 오차는 0이므로, 체계적 편향은 프라이버시 메커니즘이 아니라 그 뒤의 비음수·사후 보정 탓이라는 것입니다.[^26]
...
Abowd의 후속 논의는 한 발 더 나갑니다. 무편향 노이즈 측정값을 선거구 규모로 모으면, 음수 항이 많더라도 합리적인 선거구를 만들 수 있고, 그 집계의 분산은 구성 통계들의 분산을 더해 정확히 계산할 수 있다는 설명입니다.[^28]
...
설계자 본인이 정확도 손상의 위치를 차등 프라이버시 노이즈가 아니라 비음수 사후 보정 제약으로 콕 집었습니다.[^30] 0인 값을 0이 아닌 값으로 바꾸면서 비음수를 강제하는 시스템이라면, 과거의 억제나 스와핑을 포함해 무엇이든 같은 체계적 편향을 겪습니다.[^30]
